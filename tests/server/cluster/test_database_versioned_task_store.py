...
@pytest.mark.asyncio
async def test_get_missing_returns_none_and_missing(
    versioned_store: VersionedDatabaseTaskStore,
) -> None:
    task, version = await versioned_store.get('nope', TEST_CONTEXT)
    assert task is None
...
async def test_first_save_then_get(
    ...
    got, version = await versioned_store.get('task-abc', TEST_CONTEXT)
    assert got.id == 'task-abc'
    assert version == v1
...
async def test_update_with_matching_version_succeeds(
    ...
    got, version = await versioned_store.get('task-abc', TEST_CONTEXT)
    assert got.status.state == TaskState.TASK_STATE_WORKING
    assert version == v2
...
async def test_stale_update_raises_conflict(
    ...
    got, _ = await versioned_store.get('task-abc', TEST_CONTEXT)
    assert got.status.state == TaskState.TASK_STATE_WORKING
...
async def test_delete_removes_task(
    ...
    task, version = await versioned_store.get('task-abc', TEST_CONTEXT)
    assert task is None
    assert version.is_missing
...
async def test_save_raises_after_exhausting_retries(
    ...
    """Persistent OperationalError surfaces once retries are exhausted."""
    from unittest import mock
    ...
    with (
        mock.patch.object(
            versioned_store, '_save_once', side_effect=always_locked
        ),
...
async def test_get_raises_after_exhausting_retries(
    ...
    """Persistent OperationalError on read surfaces once retries are exhausted."""
    from unittest import mock
    ...
    with (
        mock.patch.object(
            versioned_store, '_get_once', side_effect=always_locked
        ),
        pytest.raises(OperationalError),
    ):
        await versioned_store.get('task-abc', TEST_CONTEXT)
