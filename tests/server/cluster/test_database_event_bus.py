"""Tests for `DatabaseTaskEventBus` including a multi-replica simulation."""
...
from a2a.server.cluster.database_event_bus import (
...
def _sqlite_dsn() -> str:
    ...
    name = f'eventbus_{uuid.uuid4().hex}'
...
def test_as_int_rejects_non_integer_version() -> None:
    from a2a.server.cluster.database_event_bus import _as_int
...
async def test_initialize_creates_table_and_is_idempotent(
    ...
    engine = create_async_engine(url)
    try:
        bus = DatabaseTaskEventBus(engine=engine, create_table=True)
...
# --- Integration tests (shared DB across store + bus) ---


@pytest.mark.asyncio
@pytest.mark.timeout(10)
async def test_bus_delivers_events_written_by_store(db_url: str) -> None:
    """A store writes events; a bus on a SEPARATE engine reads them.

    This simulates the producing replica (store) and a subscribing replica
    (bus) sharing one database.
    """
    store_engine = create_async_engine(db_url)
    bus_engine = create_async_engine(db_url)
    try:
        store = VersionedDatabaseTaskStore(
            engine=store_engine, create_table=False
        )
        bus = DatabaseTaskEventBus(
            engine=bus_engine, create_table=False, poll_interval_s=0.05
        )
        ...
        async def consume() -> None:
            async for ve in bus.subscribe('task-abc', after=v1):
...
async def test_publish_is_noop(db_url: str) -> None:
    bus_engine = create_async_engine(db_url)
    try:
        bus = DatabaseTaskEventBus(engine=bus_engine, create_table=False)
        await bus.initialize()
        # publish is a no-op for the DB bus; must not raise.
        await bus.publish(
            'task-abc', VersionedEvent(create_task(), TaskVersion(1))
        )
        await bus.destroy('task-abc')  # also a no-op
    finally:
        await bus_engine.dispose()
