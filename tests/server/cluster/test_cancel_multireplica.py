"""Multi-replica behavioural tests for tasks/cancel (on_cancel_task).

A cancel that lands on a replica NOT running the agent records CANCELED via
compare-and-swap; the owning replica observes the version bump on its next save
and aborts. No deadlock, and no FAILED written for a healthy remote task.
...
async def test_cancel_on_non_owning_replica_stops_remote_agent(
    ...
    try:
        ...
        final, _ = await shared_store.get(task_id, context)
...
@pytest.mark.asyncio
@pytest.mark.timeout(10)
async def test_cancel_non_owner_rejected(shared_store, shared_bus) -> None:
    handler = make_replica(shared_store, shared_bus, CompletingAgent())
    try:
        result = await handler.on_message_send(
