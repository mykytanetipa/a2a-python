...
def test_inner_property_exposes_wrapped_store() -> None:
    inner = InMemoryTaskStore()
    adapter = LegacyTaskStoreAdapter(inner)
...
async def test_get_returns_task_and_missing_version() -> None:
    ...
    retrieved, version = await adapter.get('task-abc', TEST_CONTEXT)
    assert retrieved == task
    assert version.is_missing


@pytest.mark.asyncio
async def test_get_missing_task_returns_none_and_missing_version() -> None:
    adapter = make_adapter()
    retrieved, version = await adapter.get('does-not-exist', TEST_CONTEXT)
...
async def test_save_never_raises_on_stale_prev_version() -> None:
    ...
    assert version.is_missing
    retrieved, _ = await adapter.get('task-abc', TEST_CONTEXT)
