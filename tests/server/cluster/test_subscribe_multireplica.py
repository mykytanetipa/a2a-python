"""Multi-replica behavioural tests for tasks/resubscribe (on_subscribe_to_task).

Two DefaultRequestHandlerV2 instances share one store and bus. A resubscribe
that lands on a replica NOT running the agent must serve the snapshot from the
store and tail the shared bus -- not build a local ActiveTask that hangs.

Shared scaffolding lives in conftest.py.
"""
...
async def test_resubscribe_on_non_owning_replica_streams_events(
    shared_store, shared_bus, context
) -> None:
    ...
    replica_a = make_replica(shared_store, shared_bus, agent)
    replica_b = make_replica(shared_store, shared_bus, agent)
    try:
        a_events: list = []
        ...
        # Release the agent on A -> it completes; B must observe via the bus.
        agent.release.set()
...
async def test_resubscribe_to_terminal_task_rejected(
    ...
    handler = make_replica(shared_store, shared_bus, agent)
    other = make_replica(shared_store, shared_bus, agent)
    try:
        ...
        # Terminal task cannot be resubscribed to (matches ActiveTask.start()
        # and the resubscription scenario tests); the non-owning replica
...
async def test_resubscribe_absent_task_raises_not_found(
    ...
    handler = make_replica(shared_store, shared_bus, ControlledAgent())
    try:
...
@pytest.mark.asyncio
@pytest.mark.timeout(10)
async def test_resubscribe_non_owner_rejected(shared_store, shared_bus) -> None:
