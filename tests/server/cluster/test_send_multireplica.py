"""Multi-replica behavioural tests for the message-send path.

Two DefaultRequestHandlerV2 instances share one VersionedInMemoryTaskStore and
one InMemoryTaskEventBus, simulating two replicas behind a load balancer. These
exercise the optimistic-concurrency save (no lost updates) and request-boundary
cache invalidation (no stale-snapshot clobber on multi-turn input_required).

Shared scaffolding (context, request builder, agents, replica factory) lives in
conftest.py.
"""
...
async def test_single_send_completes_on_shared_store(
    ...
    handler = make_replica(shared_store, shared_bus, CompletingAgent())
    try:
        ...
        task, version = await shared_store.get(result.id, context)
...
async def test_multiturn_input_required_across_replicas(
    ...
    try:
        ...
        stored, _ = await shared_store.get(task_id, context)
...
async def test_stale_version_save_raises_conflict_directly(
    ...
    try:
        ...
        task, v = await shared_store.get(result.id, context)
