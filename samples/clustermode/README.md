...
bus, so any replica can serve any request for any task. This demonstrates the
...
- **Shared, versioned task store** (`VersionedDatabaseTaskStore`) — concurrent
  ...
  updates.
- **Shared event bus** (`DatabaseTaskEventBus`) — a subscription on one replica
...
| File | Purpose |
|------|---------|
| `cluster_common.py` | Shared store/bus wiring, agent card, and the demo agent |
...
part is passing a shared `event_bus` and a shared `VersionedTaskStore`:
...
handler = DefaultRequestHandler(
    ...
    event_bus=DatabaseTaskEventBus(engine=..., create_table=False),
)
```

Omit `event_bus` (and use a plain `InMemoryTaskStore`) and you get the ordinary
