...
# Bounded retry for transient DB contention (lock/serialization errors) on
# both reads and writes. Counts total attempts, not retries after the first.
_MAX_ATTEMPTS = 5
_RETRY_DELAY_S = 0.02
...
class VersionedDatabaseTaskStore(VersionedTaskStore):
    ...
    def __init__(  # noqa: PLR0913
        ...
        `max_attempts` and `retry_delay_s` bound the retry of transient DB
        contention (lock/serialization errors) on both reads and writes:
        `max_attempts` is the total number of tries (not retries after the
        first), and successive backoffs are `retry_delay_s` scaled linearly by
        attempt. The defaults suit SQLite's brief write locks; tune them for the
        backend in use (e.g. more attempts under Postgres SERIALIZABLE, or fewer
        when relying on MySQL's own lock-wait timeout).
        """
    ...
    async def save(
        ...
        *,
        ...
        replay via `DatabaseTaskEventStream`.
        """
        del prev
